...
public class InanimateDoor extends InanimateBox {
	...
	public InanimateDoor(Vec2D pos) {
		super(pos, new Vec2D(1, 7), new RGB(130, 130, 130));
	}
	...
	public void render(Field f) {
		...
		for (float x = start.getX(); x <= end.getX(); x+= 0.2) {
			f.drawCircle(new Vec2D(x, start.getY()), new Vec2D(0.12f), new RGB(80, 80, 80));
			f.drawCircle(new Vec2D(x, end.getY()), new Vec2D(0.12f), new RGB(80, 80, 80));	
		}
		
		for (float y = start.getY(); y <= end.getY(); y+= 0.2) {
			f.drawCircle(new Vec2D(start.getX(), y), new Vec2D(0.12f), new RGB(80, 80, 80));
			f.drawCircle(new Vec2D(end.getX(), y), new Vec2D(0.12f), new RGB(80, 80, 80));
		}
	}
